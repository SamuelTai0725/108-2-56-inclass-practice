...
```{r ans14}
mostViolatedArticle_content <- table(badEmployers$lawContent)[c(F, F, F, F, F, F, F, F, F, F, F, F, F, F, F, F, F, F, F, F, F, F, F, T, F, F, F)]
...
popularSentenceMonth <- badEmployers$dt[26]
...
factor是從1開始的整數，沒有0喔。
```{r ans21}
factor1 <- as.factor(table$國文)
order1 <- ordered(factor1)
chinese <- as.integer(order1)
chinese
```
...
```{r ans22}
ntpu_namelist1 <- str_detect(students10$志願, "北大")
students10$姓名[ntpu_namelist1] -> ntpu_namelist
ntpu_namelist
```
...
```{r ans23}
a <- students10$"國文" >= 12
students10$英文+students10$數學 >= 26 -> b
ntpu_econtpu_eco1 <- c(ntpu_namelist[a][b])
ntpu_econtpu_eco <- ntpu_econtpu_eco1[-c(5,6)]
ntpu_econtpu_eco
```
...
```{r ans24}
paste0(c("恭喜"), ntpu_econtpu_eco) -> congrats1
paste0(congrats1, c("同學錄取國立臺北大學經濟系")) -> congrats
congrats
```
